...
- 2026-09-01（Task 6 双审查）：①规格审查 PASS（三处适配判定正当：Button→Surface+combinedClickable、振动走 Android 原生 View API、compKeys 经 onSto/onRcl 回调注入）。②质量审查备注（两处小修已做）：foundation 1.7.0 长按框架不自动振动——Keypad 的 onLongClick 补 `HapticFeedbackConstants.LONG_PRESS`；存储器键（STO/RCL/M+/M-）不消费 SHIFT（会滞留误触后续 shift 层）——已在 CompScreen 这些键的处理器补 `clearShift()`（DEL/AC/MODE 同步补）。③其余备注（非阻塞）：STO/M+ 取 Ans 而非当前显示值（真机取显示值，简化记录）；存储器操作无可视反馈（UX 注）；ripple 不被药丸形裁剪（装饰级）。
