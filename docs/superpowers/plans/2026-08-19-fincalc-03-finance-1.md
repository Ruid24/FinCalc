...
- 2026-08-19（整体终审发现，已修复）：①Cash.npv/nfv/pbp 曾缺 I%≤−100 校验（CN-168 的利率规则不限模式；I%=−100 曾静默返回 Infinity）；已加 checkRate，新增 CashTest 回归（13 测）。②Cmpd 四个 solve* 的 I%=0 捷径曾跳过 checkPyCy；已在入口统一校验，新增 CmpdTest 回归（17 测）。全量 134 测 0 败。
