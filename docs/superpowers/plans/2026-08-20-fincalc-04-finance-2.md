...
- 2026-08-20（整体终审）：PASS WITH NOTES。终审独立复算锚点全部命中（DAYS 365 → 173、BOND 例1 PRC/INT/CST 逐位一致、STAT 线性回归系数差 ≤1e-15）。新增两项待计划 5 处理事项（见下"遗留事项"）。验收标准的测试计数措辞已订正为实际值（181 测）。"core/finance 实为 11 个 object"措辞澄清：12 模式 = 11 个 finance object + COMP（core/expr 表达式引擎）。
- 2026-08-20（Task 5 双审查）：①规格审查——Stat.kt 的 KDoc 与计划有一行措辞差（实现方拆行并加"y 为 null 表示 1-VAR 数据"半句，纯注释），已将计划同步为磁盘版本。②质量审查备注（非阻塞未改码）：`exp()` 溢出静默返回 +Inf（如幂/ab 回归大系数），说明书未规定、真机应报 MATH——列入后续加固批次；FREQ 加权回归与估计值定义域守卫无测试（FREQ 加权等价性已经审查实测：freq=2 与重复两次逐位相等）；`sumX3`/`sumX4` 注释段归属（走 check1 却在 2-VAR 注释段下）可顺手澄清。
...
预期：`BUILD SUCCESSFUL`；全部测试类 0 失败 0 错误（预期总测试数 134 + 10 + 7 + 11 + 9 + 10 = 181）。
...
- [ ] `./gradlew testDebugUnitTest` 全绿（181 测：计划 3 末 134 + 新增 47）
